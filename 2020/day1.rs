fn main() {
    ...
    'outer: while i < expenses.len() {
        // seed each loop iter to start after the loc of the outer loop
        j = i + 1;
        while j < expenses.len()  {
            // ditto
            k = j + 1;
            while k < expenses.len() {
                let x = expenses[i] + expenses[j] + expenses[k];
